...
<img src="shot.png" alt="pacman" style="max-width: 320px; width: 100%;" />
...
原版是按键驱动：按一次键调一次游戏逻辑。结果就是按键越快 tick 走得越快，power-up 倒计时也跟着加速。更糟的是，如果不按键，鬼也不动——整个游戏是被动推进的。

改成了 16Hz 固定帧率。主循环不再等按键才推进，而是用 `gettimeofday` 算经过时间，每凑够一个 tick（62500μs）就执行一次 `next()`。按键在两次 tick 之间随时读入，缓存在 `pending_key` 里，等下次 tick 到期再处理：
...
改动就三点：一是游戏速度和输入解耦，按键快慢不影响倒计时；二是不按键鬼也照动，游戏节奏由帧率决定而非用户；三是 `select` 的 timeout 设成剩余等待时间，既不空转也不漏读输入。
